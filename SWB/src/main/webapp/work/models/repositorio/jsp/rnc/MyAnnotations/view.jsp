        ...
        if ($('#bodyValue').val().length > 0) {
            $.getJSON('<%=saveURL.toString()%>', {'id': '<%=id%>', 'bodyValue': $('#bodyValue').val()}, function (data) {
                ...
                lContent += '<div class="media">' +
                        '<img class="align-self-start mr-3" src="/work/models/<%=siteid%>/img/usuario.jpg">' +
...
<section id="anotaciones">
    <div class="container">
        ...
        <p><a href="/<%=lang%>/<%=siteid%>/Registro?beannotator" class="oswM"><%=paramRequest.getLocaleString("lbl_register")%></a></p> 
            <%
        ...
        <p><a href="/<%=lang%>/<%=siteid%>/Registro?beannotator" class="oswM"><%=paramRequest.getLocaleString("lbl_register")%></a></p>     
            <%
                    ...
                    String myphoto = "/work/models/"+siteid+"/img/usuario.jpg";
                    if (null != user.getPhoto() && user.getPhoto().trim().length() > 0) myphoto = SWBPortal.getWebWorkPath() + user.getPhoto();
            %>                
        <div class="media">
            <img class="align-self-start mr-3" src="<%=myphoto%>">
            <div class="media-body">
        ...
        <div id="annotationList">
            <%
                ...
                for (Annotation annotation : annotations) {
                    creatorName = "";
                    tmpphoto = "/work/models/"+siteid+"/img/usuario.jpg";
                    if (ur.getUser(annotation.getCreator()) != null) {
                        User usrcrt = ur.getUser(annotation.getCreator());
                        creatorName = usrcrt.getFullName();
                        if (null != usrcrt.getPhoto() && usrcrt.getPhoto().trim().length() > 0) {
                            tmpphoto = SWBPortal.getWebWorkPath() + usrcrt.getPhoto();
                        }
                    }
            %>
            <div class="media">
                <img class="align-self-start mr-3" src="<%=tmpphoto%>" _class="circle">
                <div class="media-body">
