...
public class MyAnnotations extends GenericAdmResource{
    ...
    public void doView(HttpServletRequest request, HttpServletResponse response, SWBParamRequest paramRequest) throws SWBResourceException, IOException {
        ...
        String site = null;//paramRequest.getWebPage().getWebSite().getId();
        String path = "/work/models/" + site + "/jsp/rnc/" + this.getClass().getSimpleName()+"/view.jsp";
        List<Annotation> annotationList;
        if (user == null) {
            annotationList = AnnotationMgr.getInstance().findByTarget(site, id, null); //orderby modified direction 1 by default
            //annotationList= AnnotationMgr.getInstance().findByTarget(id,null,"created",0); //orderby created direction 0
        }else {
            annotationList = AnnotationMgr.getInstance().findByTarget(site, id, user.getId());
        }
