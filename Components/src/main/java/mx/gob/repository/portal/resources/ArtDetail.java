...
public class ArtDetail extends GenericAdmResource {
    ...
    public void doView(HttpServletRequest request, HttpServletResponse response, SWBParamRequest paramRequest) throws IOException {
        ...
        try {
            if (null != uri) {
                ...
                request.setAttribute("serie", serie(request, entry, baseUri));
                request.setAttribute("collection", explore(request, entry, baseUri));
            }
    ...
    public void doTchdta(HttpServletRequest request, HttpServletResponse response, SWBParamRequest paramRequest) throws java.io.IOException {
        ...
        try {
            if (null != uri) {
                ...
                if (null != entry) {
                    List<Entry> serie = serie(request, entry, baseUri);
    ...
    private Map getOrderedMap(Map mapper) {
        ...
        if (mapper.containsKey("records")) {
            ...
            if (!records.isEmpty()) mapper = (Map)records.get(0);
            else return orderedMap;
        }
    ...
    private List<Entry> serie(HttpServletRequest request, Entry entry, String endPoint) {
        String key = null;
        ...
        if (null != entry.getSerie() && !entry.getSerie().isEmpty()) {
            ...
            List<Entry> records = bookCase(request, endPoint, key, false);
            for (Entry e : records) {
    ...
    private List<Entry> explore(HttpServletRequest request, Entry entry, String endPoint) {
        ...
        if (null != collection && !collection.isEmpty()) {
            for (String rack : collection) {
                List<Entry> qrack = bookCase(request, endPoint, rack, false);
        ...
        if (bookCase.size() < elements && null != entry.getHolder()) {
            for (String rack : entry.getHolder()) {
                List<Entry> qrack = bookCase(request, endPoint, rack, true);
    ...
    private List<Entry> bookCase(HttpServletRequest request, String endPoint, String rack, boolean clean) {
        ...
        String uri = endPoint + "/api/"+version+"/search?q=";
        try {
        ...
        try {
            document = getReference(request, uri);
        } catch (Exception se) {
            LOG.error(se);
        }
    ...
    private Document getReference(HttpServletRequest request, String uri) {
        Document document = null;
        try {
