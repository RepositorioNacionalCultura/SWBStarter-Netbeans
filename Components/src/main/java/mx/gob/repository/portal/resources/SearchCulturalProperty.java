...
public class SearchCulturalProperty extends PagerAction {
    ...
    public static Document getReference(HttpServletRequest request, WebSite site) {
        String filters = "";
        Document document = null;
        ...
        if (null == baseUri || baseUri.isEmpty()) baseUri = SWBPlatform.getEnv("rnc/endpointURL", "").trim();
        ...
        try {
            ...
            if (null != mapper && null != mapper.get("records")) {
    ...
    private static void setAggs(Document document, List aggsList) {
    ...
    private static List<CountName> countNameList(Map map, String filter) {
    ...
    private static String getFilters(HttpServletRequest request) throws UnsupportedEncodingException {
        StringBuilder filters = new StringBuilder();
        HashMap<String, DataObject> definition = Util.getAllDSFacetProps("Record");
        Iterator it = definition.keySet().iterator();
        while (it.hasNext()) {
    ...
    private static String getFilter(HttpServletRequest request, String att) throws UnsupportedEncodingException {
    ...
    private static String getFilterDate(HttpServletRequest request, String att) {
        StringBuilder filter = new StringBuilder();
        if (null != request.getParameter(att) && !request.getParameter(att).isEmpty()) {
            String [] filters = request.getParameter(att).split(",");
            if (filters.length == 2)
    ...
    private static String getRange(HttpServletRequest request) {
        int s = 0;
